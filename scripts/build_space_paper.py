#!/usr/bin/env python3
"""Build the space paper and HAL/arXiv artifacts."""
...
def main() -> int:
    ...
    m = re.search(r"\\begin\{abstract\}(.*?)\\end\{abstract\}", abstract_tex, re.S)
    if not m:
        raise SystemExit("[build_space_paper] ERROR: abstract not found in sections/00_abstract.tex")
    ...
    if not kw_match:
        raise SystemExit("[build_space_paper] ERROR: could not extract keywords line from main.tex")
        ...
        if m:
            try:
                if float(m.group(1)) < 1.4:
                    raise SystemExit("[build_space_paper] ERROR: PDF version is < 1.4.")
            except ValueError:
                pass
    ...
    if shutil.which("pdffonts"):
        ...
        if not_embedded:
            raise SystemExit("[build_space_paper] ERROR: non-embedded fonts detected.")
        if type3:
            raise SystemExit("[build_space_paper] ERROR: Type 3 fonts detected.")
    ...
    if shutil.which("pdftotext"):
        ...
        if re.search(r"(confidential|draft|do not distribute)", text, re.IGNORECASE):
            raise SystemExit("[build_space_paper] ERROR: draft/confidential watermark text detected.")
        ...
        if "Automorph Inc." not in page1:
            raise SystemExit("[build_space_paper] ERROR: affiliation 'Automorph Inc.' not found on page 1.")
